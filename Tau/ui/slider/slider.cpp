...
{
	if (mouse_position.x <= position.x + size.x && mouse_position.x >= position.x)
	{
		if (mouse_position.y <= position.y + size.y && mouse_position.y >= position.y)
		{
			return true;
		}
	}
...
void Slider::init_slider_shape()
{
    ...
    for (int i = 0; i < 4; ++i) 
    {
        float factor = (i == 0 || i == 3) ? 0.0f : 1.0f; // Correctly set factor based on position

        slider_shape[i].color = sf::Color(
            static_cast<sf::Uint8>((1.0f - factor) * color_left.r + factor * color_right.r),
            static_cast<sf::Uint8>((1.0f - factor) * color_left.g + factor * color_right.g),
            static_cast<sf::Uint8>((1.0f - factor) * color_left.b + factor * color_right.b)
        );
    }
}